...
def nbdev_update_lib(fname:Param("A notebook name or glob to convert", str)=None):
    "Propagate any change in the modules matching `fname` to the notebooks that created them"
    script2notebook(fname=fname)

# Cell
@call_parse
def nbdev_diff_nbs():
    "Print the diff between an export of the library in notebooks and the actual modules"
    diff_nb_script()
...
def nbdev_install_git_hooks():
    ...
    try:
        #Get hooks folder. Works whether this is a regular repo or a submodule.
        hooks_path_str = subprocess.check_output("git rev-parse --git-path hooks")
        hooks_path = Path(hooks_path_str.decode('utf-8').strip())

        #Root folder of git repo or submodule
        toplevel_path_str = subprocess.check_output("git rev-parse --show-toplevel")
        toplevel_path = Path(hooks_path_str.decode('utf-8').strip())
    except: hooks_path = toplevel_path = Path.cwd()
    fn = hooks_path/'post-merge'
...
"""
    ...
    gitconfig_path = toplevel_path/'.gitconfig'
...
#
# If you see notebooks that aren't stripped, verify the filters are applied in .gitattributes
#
[filter "clean-nbs"]
...
""")
    cmd = f"git config --local include.path {gitconfig_path}"
